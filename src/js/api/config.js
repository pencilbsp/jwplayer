...
const Config = function (options, persisted) {
    ...
    if (rateControls) {
        ...
        rates = rates
            .filter((rate) => isNumber(rate) && rate >= 0.25 && rate <= 4)
    ...
    config.bandwidthEstimate = isValidNumber(parsedBwEstimate)
        ? parsedBwEstimate
        : _adjustDefaultBwEstimate(config.defaultBandwidthEstimate);
    config.bitrateSelection = isValidNumber(parsedBitrateSelection)
        ? parsedBitrateSelection
        : Defaults.bitrateSelection;

    config.liveSyncDuration = getLiveSyncDuration(config.liveSyncDuration);

    config.backgroundLoading = isBoolean(config.backgroundLoading)
        ? config.backgroundLoading
        : Features.backgroundLoading;
    return config;
};
