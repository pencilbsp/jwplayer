...
function VideoProvider(
  ...
  _this.loadAndParseHlsMetadata =
    loadAndParseHlsMetadata === undefined ? true : !!loadAndParseHlsMetadata;
  ...
  const MediaEvents = {
    ...
    error(): void {
      ...
      if (
        ...
        _this.retries < _this.maxRetries
      ) {
        // Workaround Safari bug https://bugs.webkit.org/show_bug.cgi?id=195452
        //  and stale manifests
        _this.trigger(
          WARNING,
          new PlayerError(null, HTML5_BASE_WARNING + errorCode - 1, error),
        );
        _this.retries++;
        _videotag.load();
  ...
  function checkVisualQuality(): void {
    ...
    if (
      ...
      if (
        (!_videotag.videoWidth && !isAudioStream()) ||
        _currentQuality === -1
      ) {
        return;
      }
  ...
  function timeToPosition(currentTime: number): number {
    ...
    if (
      _this.isLive() &&
      isDvr(seekRange.end - seekRange.start, minDvrWindow)
    ) {
  ...
  function getPosition(currentTime: number): number {
    ...
    if (_this.isLive()) {
      const rangeUpdated =
        !dvrPosition || Math.abs((dvrEnd as number) - seekRange.end) > 1;
      if (rangeUpdated) {
  ...
  _this.getDuration = function (): number {
    ...
    if (
      (_androidHls && duration === Infinity && _videotag.currentTime === 0) ||
      isNaN(duration)
    ) {
      return 0;
    }
  ...
  function _getPublicLevels(levels: QualityLevel[]): { label: string }[] {
    ...
    if (Array.isArray(levels) && levels.length > 0) {
      publicLevels = levels.map(function (
        level: GenericObject,
        i: number,
      ): { label: string } {
        return {
  ...
  function _play(): Promise<void> {
    const resumingPlayback =
      _videotag.paused && _videotag.played && _videotag.played.length;
    if (
      resumingPlayback &&
      _this.isLive() &&
      !isDvr(_getSeekableEnd() - _getSeekableStart(), minDvrWindow)
    ) {
      _this.clearTracks();
      _videotag.load();
    }
  ...
  this.destroy = function (): void {
    ...
    _removeListeners(MediaEvents, _videotag);
    _this.removeTracksListener(
      _videotag.audioTracks as any,
      "change",
  ...
  function _setAudioTracks(tracks: AudioTrackList | undefined): void {
    ...
    if (tracks.length) {
      ...
      _audioTracks = map(
        tracks as any[],
        function (track: any): SimpleAudioTrack {
          const _track = {
  ...
  function _setCurrentAudioTrack(index: number): void {
    if (
      ...
      _this.trigger(AUDIO_TRACK_CHANGED, {
        currentTrack: _currentAudioTrackIndex,
        tracks: _audioTracks,
      });
    }
