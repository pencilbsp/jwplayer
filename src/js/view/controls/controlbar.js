...
import { pipSupported } from "../../utils/video";
...
function createCastButton(castToggle, localization) {
    if ("WebKitPlaybackTargetAvailabilityEvent" in window) {
        ...
        const castButton = button("jw-icon-cast", null, localization.cast);
        castButton.ui.off();
...
function buttonsInFirstNotInSecond(buttonsA, buttonsB) {
    return buttonsA.filter(
        (a) => !buttonsB.some((b) => b.id + b.btnClass === a.id + a.btnClass && a.callback === b.callback)
    );
}
...
export default class Controlbar {
    constructor(_api, _model, _accessibilityContainer) {
        ...
        if (!this._isMobile) {
            ...
            volumeGroup = new VolumeTooltipIcon(
                _model,
                "jw-icon-volume",
                vol,
        ...
        _model.change(
            ...
            (model, val) => {
                ...
                for (let i = 0; i < fullscreenElements.length; i++) {
                    ...
                    const fullscreenText = model.get("fullscreen")
        ...
        if (elements.cast && elements.cast.button) {
            const castUi = elements.cast.ui.on(
                "click enter",
                function (evt) {
        ...
        const durationUi = addClickAction(
            elements.duration,
            function () {
        ...
        const controlbarUi = new UI(this.el).on(
            "click drag",
            function () {
    ...
    updateButtons(model, newButtons, oldButtons) {
        ...
        for (let i = addedButtons.length - 1; i >= 0; i--) {
            ...
            if (newButton.id === "related") {
                ...
                firstButton =
                    buttonContainer.querySelector('[button="related"]') || this.elements.settingsButton.element();
            } else {
